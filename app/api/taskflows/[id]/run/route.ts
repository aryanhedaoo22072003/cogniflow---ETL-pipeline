...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!taskflow || taskflow.ownerId !== ownerId) return NextResponse.json({ error: "Taskflow not found" }, { status: 404 });
    ...
    for (const node of taskNodes) {
      ...
      try {
        const result = await executeAndLogPipeline(pipelineId, ownerId, `(via Taskflow: ${taskflow.name})`);
