...
export default function PipelinesPage() {
  ...
  useEffect(() => {
    safeFetchJson<{ pipelines: any[] }>("/api/pipelines").then((r) => {
      setPipelines(r.data?.pipelines || []);
      setLoading(false);
    });
  }, []);

  const filtered = pipelines.filter((p) => p.name.toLowerCase().includes(query.toLowerCase()));

  return (
    <div className="p-7 overflow-y-auto">
      <div className="text-[12px] text-[#9AA1B2] mb-2">Home <span className="mx-1">›</span> <span className="text-[#6B7385]">Data Integration</span></div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-3 gap-4">
          {filtered.map((p) => (
            <Link key={p._id} href={`/dashboard/designer/${p._id}`} className="bg-white border border-[#E3E7EF] rounded-xl p-5 hover:border-[#2F6FED] hover:shadow-sm transition-all block">
