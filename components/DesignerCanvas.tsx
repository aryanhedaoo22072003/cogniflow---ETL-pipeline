...
import AiCleaningModal from "@/components/AiCleaningModal";
...
const [showAiCleaning, setShowAiCleaning] = useState(false);
  ...
  return (
    <div className="flex flex-col h-screen bg-[#F4F6FA] overflow-hidden">
      ...
      <div className="h-12 bg-white border-b border-[#E3E7EF] flex items-center px-4 gap-3 flex-shrink-0 z-30">
        ...
        <ShareButton pipelineId={savedId || ""} />
        ...
        {/* <button onClick={runPipelineNow} disabled={running} className="text-xs font-semibold bg-[#2F6FED] text-white rounded-lg px-4 py-1.5 hover:bg-[#245BD1] disabled:opacity-50 flex items-center gap-1.5">
          ...
          <button onClick={() => { if (savedId) setShowRunProgress(true); else runPipelineNow(); }} disabled={running} className="text-xs font-semibold bg-[#2F6FED] text-white rounded-lg px-4 py-1.5 hover:bg-[#245BD1] disabled:opacity-50 flex items-center gap-1.5">
          <Play size={12} /> {running ? "Running…" : "Run pipeline"}
        </button>
      </div>
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {selected ? (
              <NodeInspector
                ...
                onOpenAiClean={() => setShowAiCleaning(true)}
              />
            ) : (
...
function NodeInspector({ node, headers, connections, allNodes, onChange, onReferenceUpload, onSourceUpload, onSourceConnection, onOpenProfile, onOpenAiSuggest,onOpenAiClean }: {
        ...
        <>
          ...
          {((cfg.mode !== "connection" && cfg.rows?.length) || (cfg.mode === "connection" && cfg.sampleRows?.length)) ? (
            <div className="flex gap-2">
              ...
              <button onClick={() => onOpenAiSuggest(node.id)} className="flex-1 text-xs font-semibold border border-[#7C6AE8] text-[#7C6AE8] rounded-lg py-2 hover:bg-[#7C6AE810]">✨ AI Suggest</button>
              <button onClick={() => onOpenAiClean()} className="flex-1 text-xs font-semibold border border-[#1FA971] text-[#1FA971] rounded-lg py-2 hover:bg-[#1FA97110]">🧹 AI Clean</button>
