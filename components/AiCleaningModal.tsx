...
// onApply: (
//   steps: { type: TransformType; config: Record<string, any> }[]
// ) => void;
//   onClose: () => void;
// }
...
export default function AiCleaningModal({
  rows,
  headers,
  onApply,
  onClose,
  ...
  async function analyse() {
    ...
    try {
      ...
      const profile = headers.map((h) => {
        ...
        const isNumeric =
          values.length > 0 && nums.length > values.length * 0.8;
        ...
        return {
          ...
          nullPct:
            values.length > 0
              ? Math.round((nulls / values.length) * 100)
              : 0,
          unique,
          isNumeric,
          sample: sample5,
      ...
      if (!res.ok) {
        throw new Error(
          `AI cleaning request failed: ${res.status}`
        );
      }
      ...
      console.error("AI cleaning analysis failed:", err);
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#F0F2F6]">
          ...
          <button
            onClick={onClose}
            className="text-[#9AA1B2] hover:text-[#1A2233]"
            aria-label="Close"
          >
            <X size={18} />
          </button>
        ...
        <div className="p-6">
          {!analysed ? (
            <div className="text-center py-8">
              ...
              <p className="text-[13px] text-[#6B7385] max-w-sm mx-auto mb-6 leading-relaxed">
                AI will scan your CSV for nulls, duplicates, inconsistent
                formats, outliers, and suggest cleaning steps to fix them.
              </p>
                ...
                {[
                  {
                    ...
                    desc: "Find and handle missing values",
                  },
                  {
                    icon: "🔄",
                    title: "Deduplication",
                  ...
                  },
                  {
                    icon: "📐",
                    title: "Format fixes",
                    desc: "Standardise dates, numbers, case",
                  },
                ].map((item) => (
                  <div
                    key={item.title}
                    className="border border-[#E3E7EF] rounded-xl p-3"
                  >
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="text-[11px] font-semibold text-[#9AA1B2] hover:underline"
                  >
                    Clear
                  </button>
                </div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <span
                          className={`text-[10px] font-semibold px-1.5 py-0.5 rounded border ${severityColor(
                            step.severity
                          )}`}
                        >
                          {step.severity}
                      ...
                      <div className="text-[11.5px] text-[#6B7385]">
                        {step.description}
                      </div>

                      <div className="text-[11px] text-[#9AA1B2] mt-1 font-mono">
                        {step.impact}
                      </div>
                    </div>
                    ...
                    <ChevronRight
                      size={14}
                      className="text-[#C5CADE] flex-shrink-0 mt-1"
                    />
                  </div>
                ))}
        ...
        {analysed && suggestions.length > 0 && (
          <div className="px-6 pb-6 flex gap-2 border-t border-[#F0F2F6] pt-4">
            <button
              onClick={onClose}
              className="flex-1 border border-[#E3E7EF] text-sm font-semibold py-2.5 rounded-xl hover:border-[#2F6FED]"
            >
              Cancel
            </button>
